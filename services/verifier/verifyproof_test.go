...
func root(input string) phase0.Root {
	tmp, err := hex.DecodeString(strings.TrimPrefix(input, "0x"))
	if err != nil {
		panic(err)
	}
	if len(tmp) != 32 {
		panic("value not 32 bytes")
	}
...
func TestVerifyProof(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "ValidatorGivenState",
			root: root("a9c660d2d401587dd5a65840ede6412a4933e78c6db95df4e8c80cd8a255d569"),
			proof: &types.Proof{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			res, err := verifier.VerifyProof(ctx, test.root, test.proof)
